...
We don't recommend attaching the package, a call to `poof::add_tricks()` in your R
...
We propose below tricks to do that. If you trigger {poof} when
...
We could have defined `poof::add_tricks("Reprex selection" = TRUE ~ call_addin("reprex", "Reprex selection"))` but then Reprex Selection would always be proposed even when it doesn't apply.
...
or `poof::show_tricks()`. You can remove tricks by calling `poof::rm_tricks()` with
...
and this would really spoil the fun of {poof}. Unless you try really hard, conditions
won't affect the global state so {poof} can be used safely.
...
`poof::add_tricks()` can take named formulas as arguments as we've seen above.
